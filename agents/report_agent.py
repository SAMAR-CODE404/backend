...
class ReportAgentNodes:
    ...
    def report_structure_creator(self, state: MnAagentState) -> MnAagentState:
        ...
        "companies": [
            state.company_a_name,
            state.company_b_name
        ],
        "sections": [
            {
                ...
                try:
                    ...
                    if isinstance(context, str):
                        try:
                            ...
                            context = {}
                    templates[company_name][section] = context.get("text", "No relevant information found.")
                except Exception as e:
    ...
    def rag_summary_generator(self, state: MnAagentState) -> MnAagentState:
        ...
        for idx, section in enumerate(state.legal_report_structure['sections']):
            ...
            for company_name in [state.company_a_name, state.company_b_name]:
                ...
                try:
                    summary = rag_instance.rag_query(
                        f"Provide a comprehensive summary for the '{section['name']}' section about {company_name}",state.retrievers[company]
                    )
            ...
            state.legal_report_structure['sections'][idx]['company_summaries'] = company_summaries
...
"""
        ...
        # Save the report
        report_filename = f"legal_report_{self.report_id}.md"
        try:
